...
An in-flight row with no child metadata invalidates the summary only when
requires_child_metadata is true, since a held row legitimately outlives its worker.
...
secondmate_home_summary_json() {  # <backlog-json> <tasks-json>
  jq -n \
    ...
    # still invalidates it. Both exclusions apply: requires_child_metadata is the
    # canonical normalized form (true only for current_role "worker", so a row
    # carrying both a hold reason and a hold kind is already excluded), and
    # hold_active additionally excludes a hold-reason-only row whose date gate has
    # not lapsed, which is the test the alarm in bin/fm-supervision-lib.sh uses.
    | ([ $owned_in_flight[]
