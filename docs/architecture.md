...
A project whose delivery does not fit those modes can carry a fleet-private custom delivery-workflow note that `bin/fm-brief.sh` uses as that ship brief's whole Definition of done in place of the generic delivery-mode one; [configuration.md](configuration.md#project-delivery-workflow-notes-dataproject-flowsnamemd) owns that convention.
