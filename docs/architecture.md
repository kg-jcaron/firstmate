...
That escalation carries a pipeline-progress reading - the shared no-mistakes daemon's cumulative CPU time and its delta since the previous escalation - because a crew blocked in one long synchronous pipeline call writes nothing in its worktree, the daemon owning those files, and renders a frozen pane while being perfectly healthy; recent worktree writes and a changing pane are therefore both absent for a healthy crew and settle nothing.
The reading is fleet-wide rather than per-crew, so it is reported for the first mate to weigh and never used to absorb, with an unchanged reading the strong direction.
Once a busy crew has passed `FM_WEDGE_DEMAND_INSPECT_COUNT` escalations the repeats move to the longer `FM_PAUSE_RESURFACE_SECS` cadence, which stays finite so a real hang cannot go silent, and applies only while it remains provably busy.
...
A durable `captain-held` transfer keeps that cadence even while its agent is live, because `bin/fm-decision-hold.sh` writes that verb only after proving a captain-held backlog row carries the decision, so the captain's visibility no longer depends on the pane waking the first mate; its recheck names the captain rather than an external wait, since a filed hold does not clear on its own.
An authoritative working verdict still outranks either declaration.
Live or inconclusive liveness remains fail-open at that initial surface for a bare `paused:`, and the secondmate idle-endpoint exemption is unchanged.

A captain-relevant status is surfaced once per distinct status line, never once per pane repaint.
An idle pane is not byte-static - a harness footer keeps its own context or token readout ticking - so keying the already-classified-stale suppressor on the pane hash alone let each tick re-open a closed episode and re-emit the bare, information-free `stale: <window>` for a status the first mate had already been woken for; with a fleet of finished-and-waiting crews the first window in recorded order consumed every cycle and the rest were never triaged.
Such a repaint is therefore decided on durable evidence instead: it joins the bounded cadence when the outcome is already routed to an independently-woken channel such as an armed PR merge poll, and otherwise runs one wedge timer, so an unacted-on captain status still escalates with its age, escalation count, and `demand-deep-inspection` marker.
The same already-surfaced test guards the native push fast-path, which escalates immediately rather than through that timer.
A different status line is always news, and a crew that went quiet without declaring anything is untouched by this dedupe and still surfaces on every repaint.
...
That reset runs after startup's own discarded-output sweeps, so the session's one full banner lands in the digest the agent actually reads rather than being spent on output nobody sees.
...
A project whose delivery does not fit those modes can carry a fleet-private custom delivery-workflow note that `bin/fm-brief.sh` injects into every ship brief ahead of the Definition of done, superseding the generic delivery-mode instructions; [configuration.md](configuration.md#project-delivery-workflow-notes-dataproject-flowsnamemd) owns that convention.
