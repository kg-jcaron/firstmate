...
Those rules bind only where a step of the note's own workflow runs the pipeline, and the brief never tells the crewmate to start a run of its own: the projects that carry a note all decline the pipeline in the note itself, so an affirmative trigger emitted after the flow would contradict it, and a per-task pipeline run stays an explicit instruction firstmate sends.
