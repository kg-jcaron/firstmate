...
When `bin/fm-brief.sh` scaffolds a ship brief for a project with a note, the note's full contract becomes that brief's Definition of done and the generic delivery-mode one is not emitted at all, so the crewmate reads exactly one completion contract; `bin/fm-spawn.sh` also prints a loud `CUSTOM_FLOW:` reminder at dispatch.
Emitting both was the 2026-08-04 defect: the generic "complete once committed" gate landed last in the brief, and crewmates followed that concrete instruction over the earlier abstract supersede clause, stopping short of the note's own draft-PR step.
Rule 1 stays derived from the delivery mode, so the injected section also states that it wins over any conflicting delivery-mode, branch, push, PR, or completion instruction elsewhere in the brief; that is what keeps a PR-opening flow on a `local-only` project from stalling against rule 1's no-push boundary.
The note is injected as a subsection of that Definition of done heading, so its own headings are demoted one level on the way in and the completion gate stays under the heading the brief's status rule points at; write the note with whatever headings read best, including underline-style ones that are rewritten in `#` form at the demoted depth, because the file on disk is never rewritten.
In `no-mistakes` mode the brief still carries the pipeline-ownership rules (drive an active run through its gates, never hand-edit or abort it, never use `--yes`, and return when it reports CI green rather than watching it until merge) alongside the custom contract, because those govern how a run is driven rather than when the task is done.
Those rules bind only where a step of the note's own workflow runs the pipeline, and the brief emits no affirmative pipeline trigger of its own: a flow that needs a pipeline run names that step itself, and a per-task run stays an explicit instruction firstmate sends.
The generic gate's `checks green` ready token stays behind with that gate, because a custom flow may run no pipeline at all and a brief must not ask for a signal its task can never emit; the completion bridge asks for the PR's URL instead.
Only the mode's completion gate is replaced; its mechanical branch-and-handover rules still reach the crewmate as their own subsection, which states that it decides nothing about completion, so a `local-only` custom-flow brief keeps the clean-fast-forward-and-rebase rule that [`bin/fm-merge-local.sh`](../bin/fm-merge-local.sh) requires and `direct-PR` keeps its merge-authority handover.
The completion bridge also asks for a PR's full `https://...` URL on the status line whenever the flow opened one, because firstmate's PR-ready step acts on that URL directly.
A custom flow usually ends at a human gate, so the brief tells the crewmate to report that handoff as `needs-decision:` and reserves the declared-external-wait verb for a wait that clears on its own such as a running deploy; reporting a review-ready handoff as a pause would leave firstmate rechecking an idle pane instead of telling the captain.
...
Promoting a scout in place to a ship task goes through the same owner: `bin/fm-promote.sh` re-scaffolds the ship instructions with `bin/fm-brief.sh --promote`, so a promoted scout on a custom-flow project gets the same injected contract as a fresh ship dispatch even though it never passes back through `bin/fm-spawn.sh`.
...
FM_GUARD_UNDISPATCHED_LIST_MAX=5   # ids listed in the guard's in-flight-with-no-worker banner; non-numeric resets to 5, 0 clamps to 1, and the count line always reports the true total
FM_COMPLETION_PENDING_MAX_AGE=86400   # seconds a teardown's completion-pending marker keeps its row out of the in-flight-with-no-worker alarm before the row surfaces again
