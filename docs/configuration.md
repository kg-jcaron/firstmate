...
The note is injected as a subsection of that Definition of done heading, so its own headings are demoted one level on the way in and the completion gate stays under the heading the brief's status rule points at; write the note with whatever headings read best, because the file on disk is never rewritten.
...
That same section also carries the instruction to run the pipeline, deferring to the note where the note replaces or forbids it, because the generic Definition of done it replaced held the only trigger that ever started a run.
