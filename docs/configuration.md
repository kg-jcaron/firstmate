...
In `no-mistakes` mode the brief still carries the pipeline-ownership rules (drive an active run through its gates, never hand-edit or abort it, never use `--yes`) alongside the custom contract, because those govern how a run is driven rather than when the task is done.
