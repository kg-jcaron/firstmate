...
| `/spec-linear`     | Spec out a Linear project or issue for any team through a collaborative question-and-answer flow, with draft review before syncing the result to Linear via the Linear MCP tools |
| `/updatefirstmate` | Self-update the running firstmate and its secondmates to the latest from origin with fast-forward-only pulls, then re-read instructions and nudge secondmates |
| `/stow`            | Sweep the session for uncaptured durable knowledge, route each finding to its disk home per AGENTS.md, file undone next steps to the backlog, and report what is now safe to reset |

`/spec-linear` is the one entry that is both: the captain can invoke it, and firstmate also loads it itself at the trigger named in [`AGENTS.md`](AGENTS.md), because a request can call for a Linear spec without naming the skill.
