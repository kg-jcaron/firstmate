...
Each claude crewmate's gitignored per-task `.claude/settings.local.json` also carries `includeCoAuthoredBy: false` alongside its turn-end Stop hook, so Claude Code's default AI co-author trailer cannot land on a project commit; `bin/fm-spawn.sh` owns that file's exact contents.

**Primary-session guard fact (verified 2026-07-04, Claude Code 2.1.201; preserved 2026-07-08, Claude Code 2.1.204).**
This is separate from the per-task crewmate turn-end hook above (that one `touch`es a marker file from a Stop hook in a task's own `.claude/settings.local.json`).
