...
Registering the hold is also what quietens the waiting worker: the durable `captain-held` transfer it writes is the watcher's proof that the captain's visibility no longer depends on that crew's idle endpoint, so an unfiled decision keeps costing repeated supervision wakes while a filed one moves to the bounded recheck cadence.
